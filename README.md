...
This script is a workaround to enable USB OTG on certain AOSP-based ROMs where OTG power and USB host mode are not properly handled by the kernel or userspace.

It manually enables OTG power and forces the USB controller into HOST mode using sysfs.

and, Its a runtime workaround for generic USB OTGs to work.
...
* If your OTG device works, but after a few seconds stops working (For example, you are moving the mouse while the script is running, and the cursor freezes and stops working) re-run the script.
